...
namespace TicketSvc.Infrastructure.Outbox
{
    public sealed class OutboxDispatcher : BackgroundService
    {
            ...
            ILogger<OutboxDispatcher> logger)
        {
            ...
            _retryPolicy = Policy
                .Handle<Exception>() // Handle all exceptions (could narrow this to specific exceptions)
                .WaitAndRetryAsync(
                    3, // Retry 3 times
                    attempt => TimeSpan.FromSeconds(Math.Pow(2, attempt)), // Exponential backoff: 2, 4, 8 seconds
                    (exception, timeSpan, retryCount, context) =>
                    {
                        _logger.LogWarning($"Retry {retryCount} of 3 failed: {exception.Message}. Waiting {timeSpan} before retry.");
                    });
        }
                    ...
                    foreach (var message in batch)
                    {
                        ...
                        try
                        {
                            ...
                            using (var scope = _services.CreateScope())
                            {
                                ...
                                // Apply retry policy when dispatching the message
